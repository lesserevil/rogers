//! Release bead creation with full metadata for audit trail.
//!
//! This module handles filing a release bead when a release is approved.
//! The release bead serves as the audit trail for the entire release process,
//! tracking branch, tag, merge commit, and GitHub Release linkage.
//!
//! ## Bead Metadata
//!
//! When a release is approved, a `chore` bead is filed with:
...
use crate::beads::client::{BeadStatus, BeadType, FileBeadRequest};

/// Metadata required to file a release bead.
///
/// This struct captures all the information needed to construct a
/// comprehensive release bead description for the audit trail.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct ReleaseBeadMetadata {
...
impl ReleaseBeadMetadata {
    /// Build a description string for the release bead that includes
...
/// Build a FileBeadRequest for a new release bead.
///
/// This constructs the complete bead filing request with all required
/// metadata fields:
/// - Title: "Release vX.Y.Z"
/// - Type: chore
/// - Tag: rodgers:type=release
...
/// - No parent (top-level bead)
pub fn build_release_bead_request(metadata: &ReleaseBeadMetadata) -> FileBeadRequest {
...
/// Build a release bead request for a release that already has a GitHub Release URL.
///
/// This is the same as `build_release_bead_request` but with the URL already
/// populated in the metadata.
#[allow(clippy::too_many_arguments)]
pub fn build_release_bead_with_url(
    version: &str,
    source_branch: &str,
    ...
    let metadata = ReleaseBeadMetadata {
    ...
    build_release_bead_request(&metadata)
}

/// Build a release bead request for a release that does not yet have a
/// GitHub Release URL (filed at release start).
#[allow(clippy::too_many_arguments)]
pub fn build_release_bead_start(
    version: &str,
    source_branch: &str,
    commits_since_last: u64,
    version_bump_reason: &str,
...
/// Update a release bead description when a GitHub Release is created.
///
/// Replaces the "Pending" placeholder with the actual URL.
pub fn update_release_bead_for_github_release(
    request: &FileBeadRequest,
    github_release_url: &str,
) -> FileBeadRequest {
...
mod tests {
    ...
    // =============================================================================
    // build_release_bead_request tests
    // =============================================================================

    #[test]
    fn test_build_release_bead_request_all_fields() {
        let metadata = ReleaseBeadMetadata {
            version: "1.2.3".to_string(),
            source_branch: "main".to_string(),
            commits_since_last: 42,
        ...
        let request = build_release_bead_request(&metadata);

        assert_eq!(request.title, "Release v1.2.3");
        assert_eq!(request.bead_type, BeadType::Chore);
        assert_eq!(request.priority, 1);
        assert_eq!(request.status, BeadStatus::Open);
    ...
    #[test]
    fn test_build_release_bead_request_priority_and_type() {
        let metadata = ReleaseBeadMetadata {
            version: "1.0.0".to_string(),
            source_branch: "main".to_string(),
    ...
    #[test]
    fn test_build_release_bead_request_no_parent() {
        let metadata = ReleaseBeadMetadata {
            version: "1.0.0".to_string(),
            source_branch: "main".to_string(),
    ...
    // =============================================================================
    // build_release_bead_with_url tests
    // =============================================================================

    #[test]
    fn test_build_release_bead_with_url_includes_github_release() {
        let request = build_release_bead_with_url(
            "2.0.0",
            "main",
            100,
            "major release",
    ...
    // =============================================================================
    // build_release_bead_start tests
    // =============================================================================

    #[test]
    fn test_build_release_bead_start_has_pending_url() {
        let request = build_release_bead_start(
            "1.5.0",
            "main",
            25,
            "feature release",
    ...
    // =============================================================================
    // update_release_bead_for_github_release tests
    // =============================================================================

    #[test]
    fn test_update_release_bead_for_github_release_replaces_pending() {
        let request = build_release_bead_start(
            "1.0.0",
            "main",
            5,
        ...
        let updated = update_release_bead_for_github_release(
            &request,
        ...
        assert_eq!(updated.bead_type, request.bead_type);
        assert_eq!(updated.priority, request.priority);
    }

    #[test]
    fn test_update_release_bead_for_github_release_already_has_url() {
        let request = build_release_bead_with_url(
            "1.0.0",
            "main",
            5,
    ...
    #[test]
    fn test_release_bead_metadata_serialization() {
        let metadata = ReleaseBeadMetadata {
            version: "1.2.3".to_string(),
            source_branch: "main".to_string(),
            commits_since_last: 42,
        ...
        let deserialized: ReleaseBeadMetadata = serde_json::from_str(&json).unwrap();
