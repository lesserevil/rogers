...
//! 2. Files backport beads per target branch
//! 3. Creates approval Discussions
//! 4. Checks approval status on existing Discussions
//! 5. Executes approved backports (branch + PR)
//! 6. Handles conflicts via conflict-resolution beads
//! 7. Marks backport beads as closed when PRs are merged

use crate::beads::controller::BeadController;
use crate::beads::schema::{bead_type, status};
...
use super::execution::{BackportConflictError, BackportExecutor, BackportResult};
...
pub struct BackportState {
    ...
    /// Pending approvals (bead_id -> approval data).
    #[serde(default)]
...
impl BackportManager {
    ...
    pub async fn run(&mut self, state: &mut BackportState) -> Result<BackportRunResult> {
        ...
        // 2. For each target branch, file a backport bead and create discussion
        ...
        for candidate in &detection_result.candidates {
            for branch in &active_branches {
                ...
                let already_backported = self
                    .is_already_backported(candidate, branch)
                    .await?;
                ...
                // File backport bead
                match self.file_backport_bead(candidate, branch).await {
                    Ok(bead_id) => {
                        result.beads_filed += 1;

                        // Create approval discussion
                        match self
                            .create_approval_discussion(candidate, branch, &bead_id, None)
                            .await
                        {
                            Ok((disc_num, disc_id)) => {
                    ...
                    Err(e) => {
                        tracing::error!("Failed to file backport bead: {}", e);
                    }
                }
            }
        }
        ...
        if !state.pending_approvals.is_empty() {
            ...
            for (bead_id, approval_result) in approval_entries {
                match approval_result {
                    ApprovalResult::Approved => {
                        if let Some(approval) = state.pending_approvals.remove(&bead_id) {
                            let exec_result = self
                                .execute_approved_backport(&approval)
                                .await;
                            ...
                            match exec_result {
                                Ok(backport_result) => {
                                    ...
                                    result
                                        .prs_created
                                        .insert(bead_id.clone(), backport_result.pr_number);
                                }
                                Err(e) => {
                                    tracing::error!(
                            ...
                            if let Err(e) = self.bead_controller.update_child_status(&bead_id, status::CLOSED).await {
                                tracing::warn!("Failed to close bead {}: {}", bead_id, e);
                            }
                        }
                    }
                    ApprovalResult::Rejected => {
                        // Mark the bead as closed with note about rejection
                        tracing::info!(
                            "Backport {} rejected by human, closing bead",
                            bead_id
                        );
                        if let Err(e) = self.bead_controller.update_child_status(&bead_id, status::CLOSED).await {
                            tracing::warn!("Failed to close bead {}: {}", bead_id, e);
                        }
                        state.pending_approvals.remove(&bead_id);
                    }
                    _ => {
        ...
        tracing::info!(
            "Backport run complete: {} candidates, {} beads, {} approvals, {} executions",
    ...
    /// File a backport bead for a candidate targeting a branch.
    async fn file_backport_bead(
        &self,
        candidate: &BackportCandidate,
        target_branch: &str,
    ) -> Result<String> {
        use crate::beads::controller::CreateChildRequest;

        let request = CreateChildRequest {
            title: candidate.bead_title(target_branch),
            description: Some(candidate.bead_description(target_branch)),
        ...
        let children = self.bead_controller.file_children("", vec![request]).await?;
...
The backport will be filed as a PR targeting **{branch}**."#,
    ...
    async fn check_pending_approvals(
        ...
        for (bead_id, approval) in &state.pending_approvals {
            match self
                .approval_manager
                ...
                .await
            {
                Ok(result) => {
                    if result != ApprovalResult::Pending {
                        results.insert(bead_id.clone(), result);
                    }
                }
                Err(e) => {
                    tracing::warn!(
    ...
    async fn execute_approved_backport(
        ...
        self.executor
            ...
            .await
            .map_err(|e| crate::error::RogersError::from(e))
    }
    ...
    async fn handle_stale_discussions(&mut self, state: &mut BackportState) -> Result<()> {
        let now = Utc::now();
        let voting_window = chrono::Duration::days(self.approval_manager.voting_window_days() as i64);
        let stale_threshold = chrono::Duration::days(self.approval_manager.stale_threshold_days() as i64);

        // Collect stale bead IDs first to avoid double mutable borrow
        ...
        for bead_id in stale_ids {
            ...
            if let Some(approval) = state.pending_approvals.remove(&bead_id) {
                ...
                if let Err(e) = self.file_revisit_bead(&approval).await {
                    tracing::warn!("Failed to file revisit bead for {}: {}", bead_id, e);
                }
            }
        }

        // Collect bead IDs needing reminders
        ...
        for bead_id in to_remind {
            if let Some(approval) = state.pending_approvals.get(&bead_id) {
                tracing::info!(
                    "Posting voting reminder for backport {}",
                    bead_id
                );
...
This discussion will be closed as stale if there is no response."#,
    ...
    /// File a revisit bead for a stale backport.
...
the backport PR and close this bead.
...
#{} (may be closed)"#,
            ...
            acceptance_criteria: Some(
                "- [ ] Backport is evaluated and either filed manually or explicitly closed".to_string(),
            ),
...
pub struct BackportRunResult {
    ...
    pub checked: usize,
    /// Backport beads filed.
    pub beads_filed: usize,
    ...
    /// PR numbers created (bead_id -> pr_number).
    #[serde(default)]
...
mod tests {
    ...
    fn test_backport_manager_github_config() {
        ...
        let auth = GitHubAuth::new_with_default_api("ghp_test_token");
