...
Rodgers detects release readiness
  ...
  └─► Creates GitHub Discussion (Release Proposal)
        ...
        └─► Waits for human reaction (👍/👎) or comment
              │
              ├─► 👍 (approval)
              │     └─► Rodgers cuts the release branch or tag
...
2. Creates a GitHub Release with:
   ...
   - Title: `{project-name} X.Y.Z`
   - Body: autogenerated from the issue tracker (linked issues, changes, breaking changes)
3. Posts a comment on the original proposal Discussion: "Release {X.Y.Z} has been cut. [Link to release]"
...
5. Rodgers creates tag + GitHub Release + release branch
